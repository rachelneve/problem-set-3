...
Which plant has the highest expression of any gene in the 6hpi time point? pps
...
cleaned_data %>% filter(time.point == "6hpi") %>% arrange(desc(value)) %>%
  head(1) %>% select(plant)
  
```

Which plant / fungus pair has the highest expression in the 18hpi time point?: pps and A6
...
cleaned_data %>%
  mutate(time.value = str_replace(time.point, 'hpi', ''),
         time.hpi = str_c('hpi.', time.value)) -> cleaned_data_hpi
...
cleaned_data_hpi %>% 
  ...
  summarise(expr.var = var(expr.diff), mean = mean(abs(expr.diff))) -> expr_data

expr_data %>%
  group_by(plant, fungus) %>% arrange(desc(expr.var)) %>% slice(3) %>% select(gene.name, plant, fungus) -> genes
...
# There are `r nrow(genes)` rows of data in `genes`. There are `r ncol(genes)` columns of data in `genes`. The most expressed genes are `r slice(3)`.
...
```{r problem_5}
semi_join(cleaned_data_hpi, genes, by = "gene.name") -> joined
  joined %>% 
  ggplot(aes(time.value, value, color = gene.name, shape = rep)) + 
    geom_point() + 
    geom_smooth(method = "gam", se = FALSE) +
    xlab('time') + 
    facet_wrap(facets = c('plant', 'fungus')) + 
    theme_bw() + 
    ggtitle('Expression.Over.Time')
```
