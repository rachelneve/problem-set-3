...
```{r problem_3}
cleaned_data_hpi %>% select(-time.point, -time.value) %>% spread(time.hpi, value) %>%
...
expr_data %>%
  arrange(desc(expr.var)) %>% unique(plant) %>% unique(fungus) %>% head(3) %>% select(gene.name, plant, fungus) -> genes

genes

#How do I select each unique plant and fungus pair here?
#How should I use slice in this context?
```
...
#Should I be summarizing the output of the most differentially expressed genes or about the data frame "expr_data?"
...
```{r problem_5}
cleaned_data_hpi[genes] %>%
  ggplot(aes(time.value, value, color = gene.name)) + geom_point()
#I'm not sure how to get the right genes from above to start plotting.
```
